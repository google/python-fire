...
from fire.parser_annotations import generate_parse_fn_annotation
...
def _DictAsString(result, verbose=False):
    ...
    result_visible = {
        ...
        if completion.MemberVisible(
            result, key, value, class_attrs=class_attrs, verbose=verbose
        )
    }
...
def _Fire(component, args, parsed_flag_args, context, name=None):
    ...
    while True:
        ...
        if not remaining_args and (
            show_help or interactive or show_trace or show_completion is not None
        ):
    ...
    if remaining_args:
        component_trace.AddError(
            FireError("Could not consume arguments:", remaining_args), initial_args
        )
        return component_trace
...
def _CallAndUpdateTrace(
    component, args, component_trace, treatment="class", target=None
):
    ...
    component_trace.AddCalledComponent(
        component, target, consumed_args, filename, lineno, capacity, action=action
    )
...
def _MakeParseFn(fn, metadata):
    ...
    def _ParseFn(args):
        ...
        for index, value in enumerate(varargs):
            varargs[index] = _ParseValue(
                value, None, None, metadata, fn_spec.annotations
            )
        ...
        if _IsFlag(argument):
            ...
            if (
                ...
                if len(matching_fn_args) == 1:
                    ...
                    raise FireError(
                        "The argument '{}' is ambiguous as it could "
...
def _ParseValue(value, index, arg, metadata, annotations):
    ...
    if arg in annotations:
        ...
        parse_fn_annotation = generate_parse_fn_annotation(annotation)
        if parse_fn_annotation:
            parse_fn = parse_fn_annotation
