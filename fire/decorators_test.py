...
class WithVarArgs(object):
    ...
    def example7(
        self, arg1, arg2=None, *varargs, **kwargs
...
class FireDecoratorsTest(testutils.BaseTestCase):
    ...
    def testSetParseFnsDefaultsFromFire(self):
        ...
        self.assertEqual(
            core.Fire(WithDefaults, command=["example1", "10"]), (10, float)
        )
        self.assertEqual(
            core.Fire(WithDefaults, command=["example1", "13"]), (13, float)
        )
        self.assertEqual(
            core.Fire(WithDefaults, command=["example1", "14.0"]), (14, float)
        )
    ...
    def testSetParseFnsNamedDefaultsFromFire(self):
        ...
        self.assertEqual(
            core.Fire(WithDefaults, command=["example2", "13"]), (13, float)
        )
        self.assertEqual(
            core.Fire(WithDefaults, command=["example2", "14.0"]), (14, float)
        )

    def testSetParseFnsPositionalAndNamed(self):
        self.assertEqual(
            core.Fire(MixedArguments, ["example3", "10", "10"]), (10, "10")
        )
    ...
    def testSetParseFnsForKeywordArgs(self):
        ...
        self.assertEqual(
            core.Fire(WithKwargs, command=["example6", "--mode", "train"]), ("train", 0)
        )
        self.assertEqual(
            core.Fire(WithKwargs, command=["example6", "--mode", "3"]), ("3", 0)
        )
        self.assertEqual(
            core.Fire(
                WithKwargs, command=["example6", "--mode", "-1", "--count", "10"]
            ),
            ("-1", 10),
        )
        self.assertEqual(
