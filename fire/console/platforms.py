    ...
    def FromId(os_id, error_on_unknown=True):
        ...
        if error_on_unknown:
            raise InvalidEnumValue(
                os_id, "Operating System", [value.id for value in OperatingSystem._ALL]
            )
        return None
    ...
    def UserAgentFragment(self):
        ...
        if self.operating_system == OperatingSystem.LINUX:
            ...
            arch_string = (
                ...
                if self.architecture == Architecture.ppc
                else "Intel"
            )
...
"""
    ...
    def IsCompatible(self, allow_py3=False, raise_exception=False):
        ...
        if not self.version:
            ...
            if self.version[0] < 3:
                ...
                if self.version < PythonVersion.MIN_REQUIRED_PY2_VERSION:
                    error = (
                        "ERROR: Python {0}.{1} is not compatible with the Google "
                        "Cloud SDK. {2}\n".format(
                            self.version[0],
                            self.version[1],
                            self.SupportedVersionMessage(allow_py3),
                ...
                if not allow_py3:
                    error = (
                        "ERROR: Python 3 and later is not compatible with the "
                        "Google Cloud SDK. {0}\n".format(
                            self.SupportedVersionMessage(allow_py3)
                        )
                    )
        ...
        if (
            self.version >= self.MIN_REQUIRED_PY2_VERSION
            and self.version < self.MIN_SUPPORTED_PY2_VERSION
        ):
            sys.stderr.write(
                """\
