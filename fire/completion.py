    ...
    for command in set(subcommands_map.keys()).union(set(options_map.keys())):
        ...
        lines.append(
            lastcommand_check_template.format(
                command=command, opts_assignment=opts_assignment
            )
        )
    lastcommand_checks = "\n".join(lines)
...
"""
    ...
    subcommand_template = (
        "complete -c {name} -n '__fish_using_command " "{command}' -f -a {subcommand}\n"
    )
    flag_template = (
        "complete -c {name} -n "
    ...
    return fish_source.format(
        global_options=" ".join(
            '"{option}"'.format(option=option) for option in global_options
        )
    )
...
def MemberVisible(component, name, member, class_attrs=None, verbose=False):
    ...
    if (
        six.PY2
        and inspect.isfunction(component)
        and name
        in (
            "func_closure",
            "func_code",
    ...
    if (
        six.PY2
        and inspect.ismethod(component)
